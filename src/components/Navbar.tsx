    ...
    <header className="site-navbar fixed left-0 right-0 top-0 z-50">
      <div className="mx-auto flex h-[58px] max-w-6xl items-stretch px-4 md:px-6">
        <NavLink to="/" className="flex items-center gap-2 no-underline">
          ...
          <span className="text-sm font-bold text-white">Анализ гемблинга</span>
        </NavLink>
        ...
        <nav className="border-t border-white/10 bg-navy-light px-4 py-3 md:hidden">
          {links.map((l) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `block py-2.5 text-sm no-underline ${isActive ? "font-semibold text-accent-light" : "text-white/70"}`
              }
            >
              {l.label}
            </NavLink>
